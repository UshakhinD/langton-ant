...
steptxt = uicontrol('Style', 'text','Position', [500,540,100,30]);
colortxt = uicontrol('Style', 'edit','Position', [400,540,100,30]);
...
clramount = length(colortxt.String);
axis tight
ant=[10,10];
Direct=0;
    function start(~,~)
        while 1
            ...
            m(ant(1),ant(2)) = mod(m(ant(1),ant(2))+1, clramount);
            p = rec(ant(1), ant(2));
            clrindx = mod()
            p.FaceColor = colors{m((ant(1),ant(2))};
